...
messages, it lives in [`src/reai/quirks.ts`](../src/reai/quirks.ts) as **116 quirks keyed to the
operations they affect** — so they surface automatically in `reai_describe_endpoint` and
`reai_search_endpoints`, including for the 152 public operations no curated tool covers.
