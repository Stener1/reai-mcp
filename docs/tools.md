...
And the asymmetry has a sharper instance than the attach one: **a supplier invoice is the only owner that takes a file directly and the only one with no detach route.** So a file attached to one cannot be detached through the API, while an order or a voucher can unlink theirs.

An earlier version of this section said an uploaded attachment "cannot be deleted at all", and that is **false** — the spec is explicit: `DELETE /api/orders/{id}/attachments/{attachmentId}` "removes the attachment from the order … **If the attachment has no other references, the stored file is deleted.**" So upload → link to an order → unlink does remove it, both steps inside the reversible tier. The same section two paragraphs up already advised asking what else references a file *before deleting*, which only makes sense on that reading; the two paragraphs contradicted each other for one PR.

`POST /api/attachments` being classified `reversible` is deliberate, and a claim that it was a misclassification — made when these tools landed — was withdrawn. `src/policy.ts` had already reasoned about the case, promoting `PATCH`/`PUT` on an attachment to irreversible *because* "no DELETE exists under /api/attachments to undo it". What the tier turns on is not deletability and not "additive" either (23 of its 88 operations are DELETEs, and 17 of its PUTs can clear a field by omission) but an **exclusion list**: nothing touching the ledger, a legal document, money, payroll, an authority filing, or user administration.
