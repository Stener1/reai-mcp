...
And the asymmetry has a sharper instance than the attach one: **a supplier invoice is the only owner that takes a file directly and the only one with no detach route**, while an uploaded attachment cannot be deleted at all. So step 1 of an upload-then-link workflow is not undoable even if step 2 fails. (`POST /api/attachments` is classified `reversible`, and **that is deliberate, not a mistake** — a correction to what this page said when the attachment tools landed. `src/policy.ts` already reasoned about exactly this case: it promotes `PATCH`/`PUT` on an attachment to irreversible *because* "no DELETE exists under /api/attachments to undo it", while recording that "UPLOADING a new attachment is additive and stays reversible". The tier turns on **additive**, not deletable. What was wrong was the tier's own one-line definition, which claimed "master data that can be cleanly deleted again"; that wording is fixed, and `test/policy.test.mjs` now holds the sentence and the reasoning together.)
