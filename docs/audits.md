...
|---|---|---|
| [`audit-quirks.mjs`](#audit-quirksmjs-the-126-claims-nobody-was-checking) | the 126 quirks, via the claims a `GET` can answer | no — safe against any tenant |
...
## `audit-quirks.mjs`: the 126 claims nobody was checking

The other two audits check `src/tools/*.ts`. **Quirks** are the third channel, and the largest: 126 of
...
`audit:census` exists to print rather than assert. The checkable figures, **restated for the 16-case audit**: **126** quirks, of which **17** now have a live
...
This covers the **16 that a GET can answer**, and the report prints `of 126` so a pass is not read as
