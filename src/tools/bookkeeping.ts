...
const listPostings = defineTool({
  ...
  inputSchema: {
    ...
    projectId: z.number().int().positive().optional().describe("Filter by project."),
    employeeId: z.number().int().positive().optional().describe("Filter by employee."),
    companyBankId: z.number().int().positive().optional().describe("Filter by company bank account."),
    tenantId: tenantIdArg,
  },
  handler: async (args, ctx) => {
    const startDate = args.startDate ?? startOfYear();
