...
| `invoiceEmail` | customers, orders, subscriptions | Every future invoice is delivered to that address. Not a payment — a disclosure — so the refusal says so, and tells you to confirm the address through a channel you already trust rather than to check bank details. **Emptying one counts too**: `invoiceEmail: ""`, and on a `PUT` leaving the field out, both stop invoices reaching the address someone chose. Measured on `PATCH /api/customers/{id}`, `""` clears the address, `null` is a no-op and `" "` is a `400` — so the form that empties a billing address is the one that looks like a typo. Naming the field with an empty value needs `full` mode either way, because the intent is the same and the endpoints do not agree. Not on a `POST`, where there is no stored address to redirect |
