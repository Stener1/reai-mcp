...
Discovery works in Norwegian, which for this API is not a nicety. Measured on one set of 29 realistic queries — 19 Norwegian, 10 English — before and after: **14 found before, 29 after**, top-three 12 → 26, and nothing ranked worse. `lønnskjøring` returned zero results while `lonn` was already a synonym; *"hvor mye lager har vi"* returned the chart of accounts.
