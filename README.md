...
- **131 measured API quirks** keyed to the operations they affect, so `reai_describe_endpoint` warns you before the API rejects you.
...
[`src/reai/quirks.ts`](src/reai/quirks.ts) as **131 quirks keyed to the operations they affect** — so
...
trusted: four audit harnesses ask whether the refusals, the stored values and the 131 quirks still
