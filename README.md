...
Measured against the live API, and the sharpest edge here fails silently. `variantId` is optional in the schema and *required in practice*: omitting it on a product that has variants answers `200` with a real `transactionId` and moves **no stock at all** — four consecutive `+3` adjustments left `quantityOnHand` at 0. `reai_adjust_inventory` reads the warehouse's stock lines first and refuses before writing, listing the variant ids to choose from, then checks the resulting quantity against what was asked for. The field to send is `variantId`; a variant in the product response is keyed `variantId`, not `id`, which is exactly how this was hit.
